---
title: "HiResTEC-vignette"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{HiResTEC-vignette}
...
common approach in flux analysis.
...
will demonstrate the general workflow.
...
belong to two groups ('A' and 'B' indicated in column `gr`) and were taken at three
different time points (0, 1 and 2). We can plot the base peak chromatograms (BPCs)
for these files to observe that they contain only a small retention time window 
(approx. 10s) and a limited mass range.

```{r show_raw_bpc, fig.dim=c(8,6), out.width="98%"}
length(raw)
class(raw[[1]])
raw[[1]]@mzrange
bpc <- lapply(raw, getMultipleBPC)
...
The highest intensity we find for m/z = 556.263. We can extract the mass 
isotopomer distribution (MID) of m/z 556.263 and its isotopes for all files. For better
visibility we omit plotting the intermediate time point and group 'B'.

```{r show_mid, fig.dim=c(8,6), out.width="98%"}
mz <- 556.263
mz_dev <- 0.4
bpc <- lapply(raw, function(x) { getMultipleBPC(x = x, mz = mz + c(0:6)*1.003355, mz_dev = mz_dev) })
flt <- sam[,"tp"] %in% c(0, 2) & sam[,"gr"] == "A"
plotBPC(bpc[flt], mfrow = c(2,3), ids = sam[flt,"ids"])
```
...
base peak chromatograms we just extracted.
...
preCL <- EvaluatePairsFromXCMSSet(xg = xg, tp = sam$tp, gr = sam$gr, dmz = 0.04)
```
...
The function finds all relevant pairs according to the limiting parameters 
(specific mass difference, within rt window, see help file of 
*EvaluatePairsFromXCMSSet()* for details) and tests each pair using an ANOVA model 
based on the group and time point information of each sample. 
...
is obtained for an m/z pair (559 and 561) within the MID of our peak. The correct 
solution would be m/z pair 556 and 561.

The task to pick the best candidate, apply rigorous quality control and avoid 
...
finCL <- EvaluateCandidateListAgainstRawData(
  x = preCL, tp = sam$tp, gr = sam$gr, dat = raw, dmz = 0.04, rolp = "all"
)
```
*EvaluateCandidateListAgainstRawData()* will result in a l*oooon*g list in a real
...
are generated per candidate. Fist, the enrichment in all samples is depicted.

```{r QCplot1, fig.dim=c(8,5), out.width="98%", echo=FALSE}
...
Time points are color coded and groups use different plotting symbols. Individual 
samples can be identified in the left subplot due to their number from the 
sample list. The annotation 'dE' in the left subplot provides the maximum 
...
In the spectrum, one should confirm that the ratio of mz1 (black) and mz2 (purple) 
are really changing. This sounds trivial, but is sometimes not the case if 
deconvolution errors occur.
...
calculated by M + 5 x 1.003355 (because 1.003355 is the difference between $^{12}C$ 
and $^{13}C$). In GC-APCI-MS (which this example is coming from), analytes are
derivatized before analysis, often using Tri-Methyl-Silyl (TMS) groups. TMS 
...
bpc <- lapply(raw, function(x) { 
  getMultipleBPC(x = x, mz = mz + c(-1:6)*1.003355, mz_dev = 0.04, rt = rt) 
})
