...
`raw` contains 18 measurements stored in a list as `xcmsRawLike` objects. The samples
...
There appears to be a peak around rt = 1025 s in all files. Let's deconvolute the
...
The base peak is observed at m/z = 556.263. We can extract the mass isotopologue 
...
preCL <- HiResTEC::EvaluatePairsFromXCMSSet(xg = xg, tp = sam$tp, gr = sam$gr, dmz = 0.04, silent = TRUE)
...
***Note!*** Intensities in chromatograms (left subplots) are depicted 
log10-transformed to enhance small signals. The spectra (right subplots) depict
measured intensities (not log10-transformed) to emphasize differences between 
time points. 

Each spectrum depicts the intensities from the scan in the left subplot indicated 
by the grey line, usually the peak center. In the spectra, one should confirm that 
the ratio of mz1 (black) and mz2 (purple) is really changing. This sounds trivial, 
but is sometimes not the case if errors in peak picking or deconvolution occurred.

Also, the small numbers at the top of each mass intensity are very informative.
They indicate the difference of the measured mass from the theoretical mass in mDa.
The theoretical mass of a M+5 isotope in a tracer experiment using ^13^C would be
calculated by *M + 5 x 1.0034* (because 1.0034 is the difference between ^12^C 
...
(0.9996 for ^29^Si and 1.9968 for ^30^Si).
...
In consequence, observing a strong negative mass deviation in non-labeled samples 
for mz2 (purple data for tp=0) and a mass deviation close to zero in labeled 
samples (tp=2) is a strong indicator for successful tracer incorporation.
...
as a possible sum formula for the above peak (Code not shown). We can use this sum 
formula to get the corrected mass isotopologue distribution as shown below.
...
shown in the `HiResTEC` QC plots above (70% and 40% at tp 2 for group A and B 
respectively) is pretty close to the results obtained by `CorMID`.
